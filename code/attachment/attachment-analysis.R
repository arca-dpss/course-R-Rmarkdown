...
my_data <- readxl::read_excel("data/data-attachment.xlsx")
...
summary(my_data) # non ci sono più NA

# Seleziono solo i soggetti che hanno il consenso, da includere e che erano presenti
my_test <- my_data$consenso=="1" & my_data$escludere=="0" & my_data$presenza=="1"
my_data<-my_data[my_test, ]
...
my_data[my_data$raccolta=="2", item_fas] <- my_data[my_data$raccolta=="2", item_fas] - 1
...
fas_tot<-apply(my_data[ , item_fas], MARGIN = 1, sum)
...
ssm_to_rev <- paste0("ssm", c(1,3,4,9,10,13,15))
# Ricodifico variabili con scoring inverso
my_data[ , ssm_to_rev] <- abs(my_data[ssm_to_rev] - 5)
...
# Ricodifico variabili con scoring inverso
sdq_to_rev <- paste0("sdq", c(7, 11, 14, 21, 25))
my_data[ , sdq_to_rev] <- abs(my_data[ , sdq_to_rev] - 2)
...
my_data<-cbind(my_data[ , c("id", "classe", "genere", "eta")],
